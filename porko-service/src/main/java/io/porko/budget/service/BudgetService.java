...
public class BudgetService {
    ...
    public BudgetResponse getBudget(YearMonth goalDate, Long memberId) {
        Budget budget = budgetRepo.findByGoalDateAndMemberId(goalDate, memberId).orElseThrow(() -> new BudgetException(BudgetErrorCode.NOT_FOUND, goalDate, memberId));
        BigDecimal totalCost = historyQueryRepo.calcTotalCost(goalDate, memberId).orElse(BigDecimal.ZERO);

        return BudgetResponse.of(totalCost.divide(budget.getGoalCost()).multiply(BigDecimal.valueOf(100)).abs());
    }
}
